...
def one_anomaly_analysis(spatial_data):
    ...
    for index in indexes:
        l_index = linear_index(0, *index[0])
        buckets[l_index] += 1
    return buckets
...
def multi_anomaly_analysis(spatial_data, dimension):
    ...
    for index in indexes:
        ...
        l_indexes = np.array([linear_index(0, *np_index[i, :]) for i in range(np_index.shape[0])])
...
def main():
    ...
    for dimension in dimensions:
        if dimension == 1:
            ...
            pass
    with open("S1_sigmoid_spatial_model.json", "w") as json_file:
